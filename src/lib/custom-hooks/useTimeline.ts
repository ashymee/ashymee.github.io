...
import { generateArray, getSortedData, getTimelineBorders } from "../timeline";
...
export const useTimeline = (
  ...
  const numberYearsInTimeline =
    endDateTimeline.getFullYear() - startDateTimeline.getFullYear() + 1;
  ...
  const years = useMemo(
    () =>
      generateArray(numberYearsInTimeline, (_, index) => {
        if (order === "desc") {
          return (
            startDateTimeline.getFullYear() + numberYearsInTimeline - 1 - index
          );
        }
  ...
  const timeline = useMemo(
    () => years.flatMap((year) => months.map((month) => ({ year, month }))),
    [months, years]
  );
  ...
  const processedData = useMemo(
    () =>
      sortedData.reduce(
        (accumulator: ProcessedTimelineData[], currentItem: TimelineData) => {
          ...
          occupiedColumns.splice(
            startMonthIndex,
            duration,
            ...updatedOccupiedColumns
          );

          const startRowGrid =
            order === "desc"
              ? timeline.length - startMonthIndex + 1
              : startMonthIndex + 1;
          const endRowGrid =
            order === "desc"
              ? timeline.length - endMonthIndex
              : endMonthIndex + 2;
  ...
  const getFormattedDateString = useCallback(
    (value: string) => {
      const text = new Date(value).toLocaleString(language, {
        month: "short",
        year: "numeric",
      });
