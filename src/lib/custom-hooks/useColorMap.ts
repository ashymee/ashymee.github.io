import type { AccentColors, Groups, ProcessedTimelineData } from "@/index";
import { hexToHsl, notNullOrUndefined } from "../timeline";
...
const generateColorVariants = (
  ...
  const variantHue =
    (parseInt(baseHSLColor.hue) + (number - 1) * hueRange) % 360;
  const variantSat = Math.max(
    0,
    Math.min(
      100,
      parseInt(baseHSLColor.saturation) + (number - 1) * saturationRange
    )
  );
  const variantLight = Math.max(
    0,
    Math.min(
      100,
      parseInt(baseHSLColor.lightness) + (number - 1) * lightnessRange
    )
  );
