import type { HexToHslReturn, TimelineData } from "..";
...
export const getTimelineBorders = (data: TimelineData[]) => {
  ...
  const periodStartingYears = data
    .map((item) => getYear(item.startPeriod))
    .filter(notNullOrUndefined);
  const periodEndingYears = data
    .map((item) => getYear(item.endPeriod))
    .filter(notNullOrUndefined);

  return {
    minYear: Math.min(...periodStartingYears),
    maxYear: Math.max(...periodEndingYears),
  };
};

export const generateArray = <T>(
  length: number,
  cb: (value: unknown, index: number) => T
) => Array.from({ length }, cb);

export const getSortedData = (data: TimelineData[]) =>
  data.sort((a, b) => a.startPeriod.localeCompare(b.startPeriod));
